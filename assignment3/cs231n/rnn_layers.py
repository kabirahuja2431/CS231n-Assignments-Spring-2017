...
def rnn_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        ...
        dx0, dh_prev, dWx0, dWh0, db0 = rnn_step_backward(dh_current,cache[t])
        dx[t] += dx0
        dWx += dWx0
        dWh += dWh0
        db += db0
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    """
    dx, dh, dc, dWx, dWh, db = 0, 0, 0, 0, 0, 0
    #############################################################################
    ...
    dtanhc_next = dnext_h*o
    dnext_c += (1-np.tanh(next_c)**2)*dtanhc_next
    dprev_c = f*dnext_c
    df = prev_c*dnext_c
    di = g*dnext_c
    dg = i*dnext_c
...
def lstm_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        ...
        dc_current = dprev_c
        dx_t, dprev_h, dprev_c, dWx_t, dWh_t, db_t = lstm_step_backward(dh_current,dc_current,cache[t])
        dx[t] += dx_t
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
