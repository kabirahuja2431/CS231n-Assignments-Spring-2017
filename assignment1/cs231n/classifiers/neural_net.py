...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    #############################################################################
    pass
    C= np.max(scores,1).reshape(N,1)
    exp_scores = np.exp(scores-C)
    W2_trans = W2.transpose()
    correct_W2 = W2_trans[y]
    correct_scores = np.sum(l1*correct_W2,1)
    
    correct_exp_scores = np.exp(correct_scores-C.reshape(N,))
    ...
    dl1[a1<=0] = 0
    dW1 = np.dot(X.T,dl1)/N
    dW1 += 2*reg*W1
    grads['W1'] = dW1    
    grads['b1'] = np.sum(dl1,0)/N     
  ...
  def predict(self, X):
    ...
    ###########################################################################
    pass
    l1 = np.dot(X,self.params['W1'])
    l1[l1<=0] = 0
    l2 = np.dot(l1,self.params['W2'])
    l2_exp = np.exp(l2)
    probs = (l2_exp).T/np.sum(l2_exp,1)
    probs = probs.T
